...
RSpec.describe Aptimizer::Lexer do
  context "Argument" do
    ...
    it "lexes a list of multi-characters arguments" do
      lexed_arr = Aptimizer::Lexer::lex("aaaa, bbbb, cccc")
  ...
  context "Predicate" do
    it "lexes a predicate" do
      lexed_arr = Aptimizer::Lexer::lex("a(a)")
      expect(lexed_arr.join(" ")).to eq("PUB ARG(a) RP EOS")

      lexed_arr = Aptimizer::Lexer::lex("h(aa)")
  ...
  context "Initial" do
    it "lexes a list of predicates" do
      lexed_arr = Aptimizer::Lexer::lex("a(a) & h(bb) & e(a,b)")
  ...
  context "Modifier" do
    ...
    it "lexes a modifier and a predicate" do
      lexed_arr = Aptimizer::Lexer::lex(".+h(bbb)")
  ...
  context "Probability" do
    it "lexes a probability" do
      proba = Aptimizer::Lexer::lex("0.45")
      expect(proba.join(" ")).to eq("PROBA(0.45) EOS")
    end

    it "lexes 1.0" do
      proba = Aptimizer::Lexer::lex("1.0")
      expect(proba.join(" ")).to eq("PROBA(1.0) EOS")
    end

    it "lexes 1" do
      proba = Aptimizer::Lexer::lex("1")
      expect(proba.join(" ")).to eq("PROBA(1.0) EOS")
    end

    it "does not lex a probability > 1.0" do
      expect {Aptimizer::Lexer::lex("1.4")}.to raise_error
    end
  end

  context "Rules" do
    it "lexes a OR" do
      r = Aptimizer::Lexer::lex("|")
