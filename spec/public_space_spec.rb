...
RSpec.describe Arg2MOMDP::PublicSpace do
  it "checks the initial state coherence" do
    ...
    agent = Arg2MOMDP::PublicSpace.new(%w(a b c), [p3], [p1, p2, p3])
  ...
  it "checks the errors on the initial state" do
    ...
    expect {Arg2MOMDP::PublicSpace.new(%w(a b c), [], [p4])}.to raise_error
    expect {Arg2MOMDP::PublicSpace.new(%w(a b c), [], [p6])}.to raise_error
    expect {Arg2MOMDP::PublicSpace.new(%w(a b c), [], [p7])}.to raise_error
    expect(Arg2MOMDP::PublicSpace.new(%w(a b c), [p7], [p7]).initial_state.size).to eq(1)
    expect(Arg2MOMDP::PublicSpace.new(%w(a b c), [], [p5]).initial_state.size).to eq(0)
    expect {Arg2MOMDP::PublicSpace.new(%w(a b c), [], [p5])}.to_not raise_error
  end
end