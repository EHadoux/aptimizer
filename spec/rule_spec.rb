...
RSpec.describe Aptimizer::Rule do
  before(:each) do
    @p1  = instance_double("Aptimizer::Predicate", :to_s => "a(a)")
    @p11 = instance_double("Aptimizer::Predicate", :negate => @p1, :to_s => "!a(a)")
    allow(@p1).to  receive(:negate) { @p11 }
    allow(@p11).to receive(:negate) { @p1  }
    @p2  = instance_double("Aptimizer::Predicate", :to_s => "e(a,b)")
    @p22 = instance_double("Aptimizer::Predicate", :to_s => "!e(a,b)")
    allow(@p2).to  receive(:negate) { @p22 }
    allow(@p22).to receive(:negate) { @p2  }
    @p3  = instance_double("Aptimizer::Predicate", :to_s => "a(b)")
    @p33 = instance_double("Aptimizer::Predicate", :to_s => "!a(b)")
    allow(@p3).to  receive(:negate) { @p33 }
    allow(@p33).to receive(:negate) { @p3  }
    @p4  = instance_double("Aptimizer::Predicate", :to_s => "h(b)")
    @p44 = instance_double("Aptimizer::Predicate", :to_s => "!h(b)")
    allow(@p4).to  receive(:negate) { @p44 }
    allow(@p44).to receive(:negate) { @p4  }
    @a1  = instance_double("Aptimizer::Alternative", :probability => 1, :to_s => "1.0: -a(a)",
                           :modifiers => [instance_double("Modifier", :type => :rem, :predicate => @p1, :to_s => "-#{@p1}")])
    @a2  = instance_double("Aptimizer::Alternative", :probability => 0.5, :to_s => "0.5: +a(a) & .-h(b)",
                  :modifiers => [instance_double("Aptimizer::Modifier", :type => :add, :predicate => @p1, :to_s => "+#{@p1}"),
                                 instance_double("Aptimizer::Modifier", :type => :rem, :predicate => @p4, :to_s => ".-#{@p4}")])
    @a3  = instance_double("Aptimizer::Alternative", :probability => 0.5, :to_s => "0.5: +e(a,b) & +a(b)",
                  :modifiers => [instance_double("Aptimizer::Modifier", :type => :add, :predicate => @p2, :to_s => "+#{@p2}"),
                                 instance_double("Aptimizer::Modifier", :type => :add, :predicate => @p3, :to_s => "+#{@p3}")])
  end
