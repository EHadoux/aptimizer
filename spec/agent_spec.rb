...
RSpec.describe Arg2MOMDP::Agent do
  it "checks the initial state coherence" do
    ...
    agent = Arg2MOMDP::Agent.new(%w(a b c), [], [p1, p2, p3])
  ...
  it "checks the errors on the initial state" do
    ...
    expect {Arg2MOMDP::Agent.new(%w(a b c), [], [p4])}.to_not raise_error
    expect(Arg2MOMDP::Agent.new(%w(a b c), [], [p4]).initial_state.size).to eq(0)
    expect {Arg2MOMDP::Agent.new(%w(a b c), [], [p5])}.to raise_error
  end
  ...
  it "properly defines actions" do
    ...
    r2 = instance_double("Rule", :premisses => [],
                         :alternatives => [instance_double("Alternative", :to_s => "-a(a)",
                                                           :probability => 1.0, :probability= => 1.0,
                                                           :modifiers => [])])
    agent = Arg2MOMDP::Agent.new(%w(a b c), [r1, r2], [])
  ...
  it "properly defaults the action names" do
    ...
    agent = Arg2MOMDP::Agent.new(%w(a b c), [r1, r2], [], [[],[]], %w(adda addb addc))
    expect(agent.action_names).to eq(%w(adda addb addc))
    expect { Arg2MOMDP::Agent.new(%w(a b c), [r1, r2], [], [[],[]], %w(adda addb)) }.to raise_error
  end
end
